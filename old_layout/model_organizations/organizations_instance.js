...
function renderRelated(title, containerId, items) {
    ...
    items.forEach(item => {
        const attrs = item.attributes || item; // fallback for flat JSON
...
if (isNaN(id)) {
    ...
    .then(res => res.json())
    .then(org => {
        const attrs = org.data?.attributes || org; // <-- FIX: fallback to flat JSON
        if (!attrs) {
        ...
        container.innerHTML = `
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <h2>${attrs.name || 'Unknown Organization'}</h2>
        ...
        fetch('https://fosterfledging.me/api/counseling?page[size]=100', { headers: { 'Accept': 'application/vnd.api+json' } })
            ...
            .then(data => {
                ...
                const nearbyCounseling = allItems.filter(i => {
                ...
                renderRelated('Nearby Counseling', 'related-counseling', nearbyCounseling);
            })
            .catch(err => {
        ...
        fetch('https://fosterfledging.me/api/housing?page[size]=100', { headers: { 'Accept': 'application/vnd.api+json' } })
            ...
            .then(data => {
                const allItems = Array.isArray(data.data) ? data.data : [data.data];
                const nearbyHousing = allItems.filter(i => {
                    const lat = parseFloat(i.attributes?.lat);
                    const lng = parseFloat(i.attributes?.lng);
                    return !isNaN(lat) && !isNaN(lng) && isNearby(instanceLat, instanceLng, lat, lng);
                });
                renderRelated('Nearby Housing', 'related-housing', nearbyHousing);
            })
            .catch(err => {
